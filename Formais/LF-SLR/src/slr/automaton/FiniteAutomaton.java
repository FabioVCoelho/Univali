...
public class FiniteAutomaton {
	...
	/**
	 * Construção do alfabeto utilizando as transições.
	 */
	...
	public List<FiniteAutomaton> unionView(FiniteAutomaton automaton) {
		int stateNumber = 1;
	...
	/**
	 * @param automaton autômato finito.
	 * @return lista de autômatos referentes à diferença.
	 */
	public List<FiniteAutomaton> difference(FiniteAutomaton automaton) {
		List<FiniteAutomaton> automata = new ArrayList<FiniteAutomaton>();

		FiniteAutomaton a = this;
		FiniteAutomaton b = automaton;
		FiniteAutomaton aComp = a.complement();
		FiniteAutomaton union = aComp.union(b);
		FiniteAutomaton difference = union.complement();
		...
		int index = difference.name.indexOf('-');
		if(index > 0)
			difference.name = difference.name.substring(0, index) + "-Diferença(" + name1 + ", " + name2 + ")";
		else
			difference.name += "-Diferença(" + name1 + ", " + name2 + ")";			

		automata.add(aComp);
		automata.add(union);
		automata.add(difference);	
